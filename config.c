...
setup_fwd_config_of_each_lcore(struct fwd_config *cfg)
{
	 ...
	 */
	nb_fs_per_lcore = (streamid_t) (nb_fs_per_lcore + 1); // n+1
...
rss_fwd_config_setup(void)
{
	...
	for (sm_id = 0; sm_id < cur_fwd_config.nb_fwd_streams; sm_id++) {
		...
		fs->retry_enabled = retry_enabled;// 这里有retry_enabled
		rxp++;
		if (rxp < nb_fwd_ports)
			continue;
		rxp = 0;
 ...
 */
static void
streams_fwd_config_setup(void)
{
	portid_t   p;
	queueid_t  q[RTE_MAX_ETHPORTS];
	queueid_t  nb_q;
	streamid_t  sm_id=0;

	cur_fwd_config.nb_fwd_lcores = (lcoreid_t) nb_fwd_lcores;
	cur_fwd_config.nb_fwd_ports = nb_fwd_ports;
	cur_fwd_config.nb_fwd_streams =nb_fwd_streams;
	...
	p = 0; 
	for (nb_q=0;nb_q<RTE_MAX_ETHPORTS;nb_q++){
		q[nb_q]=0;
	}
	...
	for (sm_id=0;sm_id<nb_fwd_streams;sm_id++){
			struct fwd_stream * fs;
			fs = fwd_streams[sm_id]; 
			p=streams_port[sm_id];
		    fs->rx_port = p;
		    fs->rx_queue = q[p];
		    fs->tx_port = p;
		    fs->tx_queue = q[p];
		    fs->peer_addr = sm_id; // stored the streams id
		    fs->retry_enabled = retry_enabled;
			printf("[Streams] stream=%d, tx_port=%d,rx_port=%d,tx_queue=%d,rx_queue=%d\n",sm_id,p,p,q[p],q[p]);
			q[p]++;
	}
	// for (sm_id = 0; sm_id < cur_fwd_config.nb_fwd_streams; sm_id++) {
	// 	struct fwd_stream *fs;
	...
	// 	fs = fwd_streams[sm_id]; 
	// 	fs->rx_port = fwd_ports_ids[p];
	// 	fs->rx_queue = q;
	// 	fs->tx_port = fwd_ports_ids[p];
	// 	fs->tx_queue = q;
	// 	fs->peer_addr = fs->tx_port;
	// 	fs->retry_enabled = retry_enabled;// 这里有retry_enabled
	// 	q++;
...
/* 
	verbose: 1 只设置rx_dump_callbacks,移除tx dump callback
			 2 只设置tx_dump_callbacks,移除rx dunp callback
			 >2 设置rx和tx dump callbacks
			 0 移除所有dump callbacks
 */
void
